...
# main function
# info from text p.43
#' implementation of ADMM algorithm with Lasso penalty
#' @param A design matrix
#' @param b response vector
#' @param lambda overrelaxation paramter
#' @param rho augmented Lagrangian parameter
#' @param iter max number of iterations
#' @param e.abs absolute tolerance stopping criterion
#' @param e.rel relative tolerance stopping criterion
#' @return a list consisting of the iteration history
#' @export
#' @examples
...
#' }
#'
#' b <- A%*%x0 + sqrt(0.001)*matrix(stats::rnorm(m))
#'
#' # set regularization term lambda
#' lambda <- 0.1*base::norm(t(A)%*%b, "F")
#'
#' rho <- 1
#' ADMM_lasso(A,b,lambda,rho)
